...
echo $timestamp
FILEPATH=./n2nresult/$1-612-$timestamp.txt
REPORT=./report/$1-612-$timestamp.csv
...
function wrk_test()
{
	...
	do      
		...
		echo "wrk -t 40 -c 100 -d30s -R ${i} -file_k 612 --latency $URL1"

		#echo "wrk -t 20 -c 100 -d30s -R ${i} -file_k 11 --latency $URL1" >> $FILEPATH
		#wrk -t20 -c100 -d30s -R${i} --latency $URL1 >> $FILEPATH
		#sleep 10

		echo "wrk -t 40 -c 100 -d30s -R ${i} -file_k 612 --latency $URL1" >> $FILEPATH

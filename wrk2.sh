...
do      
	...
	do
		...
		do
			echo "wrk -t${j} -c${k} -d30s -R${i} --latency $URL1"
			echo "wrk -t${j} -c${k} -d30s -R${i} --latency $URL1" >> $FILEPATH
	...
	for j in 4 10 20
        do
                for k in  20 100 500 1000
                do
                        echo "wrk -t${j} -c${k} -d30s -R${i} --latency $URL2" >> $FILEPATH
	...
	for j in 4 10 20
        do
                for k in  20 100 500 1000
                do
                        echo "wrk -t${j} -c${k} -d30s -R${i} --latency $URL3" >> $FILEPATH
